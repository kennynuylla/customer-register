...
namespace WebAPI.Controllers
{
    ...
    public class PhoneController : ApiControllerBase
    {
        ...
        /// <response code="500">An error occurred</response>
        [HttpPut("{uuid}")]
        public async Task<ActionResult> Update(Guid uuid, UpdatePhoneModel model)
        {
            var (phone, customerUuid) = model.GetPhone(uuid);
            var result = await _phoneService.SaveAsync(phone, customerUuid);
        ...
        {
            var result = await _phoneService.DeleteAsync(uuid);
            if (!result.IsSuccessful) return FailResult(result);
            if (await _unitOfWork.SaveChangesAsync()) return NoContent();
            return ErrorResult();
        }
